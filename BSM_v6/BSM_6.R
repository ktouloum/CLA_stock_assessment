 ...
 # Final_stock <- Final_stock %>% debounce(1000)
  ...
  Catch_obj_=reactive({
   ...
   #   req(temp_rpriors())
    ...
    observe({
      req()
      updateSliderInput(session, "man_Bk_int_year_A", value =as.integer(floor((Catch_obj_()[["ct_data"]][c("yr","ct_smthd")]$yr[which(Catch_obj_()[["ct_data"]][c("yr","ct_smthd")]$ct_smthd>0)[1]]+max(as.integer(Catch_obj_()[["ct_data"]]$yr),na.rm = T))/2)),min = min(Catch_obj_()[["ct_data"]]$yr), max =max(Catch_obj_()[["ct_data"]]$yr))
    })
