    ...
    "))
      ...
      shinydashboard::tabItem(tabName = "run_model",
                                       ...
                                       shiny::fluidRow(
                                         conditionalPanel(condition = "input.Start_run",
                                                          shinydashboard::box(collapsible = F,
                                                                              ...
                                                                              div(
                                                                                style = "height: 100px; overflow-y: auto; padding-top: 5px;",
                                                                              shiny::h4("The plot shows as black curve the catch and as dashed horizontal line the estimate of MSY, both with indication of 95% CI. The dots indicate the catch data provided to the model."
                                                                                        ))),
                                                                              width = 12,height = 620))),
                                       shiny::fluidRow(
                                         conditionalPanel(condition = "input.Start_run",
                              ...
                              shinyBS::bsModal(id='modalExample',
                                               ...
                                               size = "large",
                                               appendToBody = TRUE,
                                               shinycssloaders::withSpinner(shiny::plotOutput('PriorPosterior', height = "70vh", width = "100%")),
                              ...
                              shinyBS::bsModal(id='popup_Page_1',
                                               ...
                                               size = "large",
                                               appendToBody = TRUE,
                                               shinycssloaders::withSpinner(shiny::plotOutput('retro_pics', height = "70vh", width = "100%")),
                              ...
                              shinyBS::bsModal(id='modalExample3',
                                               ...
                                               size = "large",
                                               appendToBody = TRUE,
                                               shinycssloaders::withSpinner(shiny::plotOutput('Diagnostics_plot', height = "70vh", width = "100%")),
                              ...
                              shinyBS::bsModal(id='modalExample5',
                                               ...
                                               size = "large",
                                               appendToBody = TRUE,
                                               shinycssloaders::withSpinner(shiny::plotOutput('Alexandros5', height = "70vh", width = "100%"))),
                              shinyBS::bsModal(id='modalExample79',
                                               title ='Summary plot',
                                               trigger="button_summary",
                                               size = "large",
                                               appendToBody = TRUE,
                                               shinycssloaders::withSpinner(shiny::plotOutput('Summary_plot', height = "70vh", width = "100%")))
                              ),
...
shinyServer=function(input, output, session){
  ...
  output$Biomass_plot2= shiny::renderPlot({
    fs <- responsive_font("Biomass_plot2", session)
  ...
  output$Biomass_plot= shiny::renderPlot({
    ...
    dt=TEST_DATA[names(TEST_DATA)==input$txt1]
    dt=dt[[1]][["input"]][["Input_data"]]
    fs <- responsive_font("Biomass_plot", session)
 ...
 # output$see_stockID=renderTable({Stock_obj$Catch_ID})
  ...
  output$loaded_biomass_plot <-renderPlot({
    req(loaded_stock_object())
    fs <- responsive_font("loaded_biomass_plot", session)
 ...
 # Final_stock <- Final_stock %>% debounce(1000)
  ...
  Catch_obj_=reactive({
    ...
    output$Check_Biomass_plot= shiny::renderPlot({
      fs <- responsive_font("Check_Biomass_plot", session)
      
      validate(need(!all(is.na(bio_obj()$bt_data$bt)), 'No CPUE data available'))
      req(bio_obj())
      xxx=bio_obj()$pic+theme_classic(base_size = fs)+
        ggplot2::theme(legend.position="bottom")
      print(xxx)
    }, height = 580)
   ...
   #   req(temp_rpriors())
    ...
    output$kiel_plot= shiny::renderPlot({
      fs <- responsive_font("kiel_plot", session)
      xxxx= run_pictures$pic_M+
        ggplot2::theme_classic(base_size = fs)+
        ggplot2::theme(legend.position="bottom")
      xxxx
    })
...
pic_F_ready <- showNotification(paste("Message: ", "Kobe plot graph ready"), duration = 5)
    ...
    observeEvent(input$Start_run, {
      ...
      CLA_object <- CLA_object_final()
        ...
        dir.create(paste0(dir.name(),"/BSM_wd/outputs/",nm),showWarnings = F)
        
       device_="png"
      ggsave(filename=paste0(paste0(dir.name(),"/BSM_wd/outputs/",nm,"/","diagnostics."),device_),plot=run_pictures$pic_I, device =device_, width = 16, height =10, units = "cm",  dpi = 300)
    ...
    observeEvent(input$Start_run, {
      ...
      out_file <- file.path(
        paste0(dir.name(),"/BSM_wd/outputs/",nm,"/"),
