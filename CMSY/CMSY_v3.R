...
library(htm2txt)
library(stringi)
library(imputeTS) ##### mipws de xreiazetai???
library(neuralnet)
library(coda)
library(tibble)
library(gplots)
...
path=getwd()
  ...
  load("aux_data.RData")
#save(species_DB,test_CATCH_data,test_CATCH_ID_All, file="aux_data.RData")
 ...
 # source("ABC_aux_functions.R")
  ...
  FFmsy[t] <- ifelse(BBmsy[t]<0.5,F[t]/(Fmsy*2*BBmsy[t]),F[t]/Fmsy)
    ...
    cat(Model, file="r2jags_test.bug")
    ...
    jags_outputs <- jags.parallel(data=jags.data,
                                  ...
                                  model.file="r2jags_test.bug", n.chains = n.chains,
 ...
 fbsb <- function(sp_name) {
  ...
  shinyUI <- shinydashboard::dashboardPage(#skin = "purple",
    shinydashboard::dashboardHeader(title = "CMSY"),
    ...
    shinydashboard::dashboardBody(
      shinydashboard::tabItems(
        ...
        shinydashboard::tabItem(tabName = "main_page",
                                shiny::fluidRow(
                                  column(width=8,offset=0.5,shinydashboard::box(title = "CMSY",  solidHeader = TRUE,status="primary",
                                                                                shiny::h4( "CMSY is a data-limited stock assessment method using a Bayesian implementation of a modified Schaefer model to predict stock status and exploitation from catch data plus priors for resilience."),
                                                                                shiny::h4( "An Artificial Intelligence neural network assists in selecting appropriate priors for relative stock size based on patterns in catch time series."),
                                                                                width=10))),
                                shiny::fluidRow(
                                  ...
                                  column(width=4,
                                         conditionalPanel(condition ="input.Upld_butt_1",
                                                          shinydashboard::box(title = "Data load",  collapsible = TRUE,
                                                                              ...
                                                                              width=12),
                                                          conditionalPanel(condition ="input.Yr_col!='' & input.ct_col!=''",#& (input.bt_col!=''| input.is_there_biomass=='Catch')
                                  ...
                                  column(width = 8,
                                         ...
                                         shiny::fluidRow(
                                           shinydashboard::box(title = "Catch plot", solidHeader = TRUE,
                                                               shinycssloaders::withSpinner(shiny::plotOutput("Catch_plot")),width = 12)#,,height=300
                                         )))
        ),
        ################################################################
        ###################### PAGE 4 Prepare Data  ####################
                                                                          ...
                                                                          shinydashboard::box(collapsible = F,
                                                                                              conditionalPanel(condition = "input.Priors_but2",
                                                                                                               ...
                                                                                                               conditionalPanel(condition = "input.upMaterial6",
                                                                                                                               ...
                                                                                                                               # awesomeCheckbox(
                                                                                                                               #   inputId = "adopt_ind",
                                                                                                                               #   label = "Adopt ANN indermediate prior",
                                                                                                                               #   value = FALSE
                                                                                                                               # ),
                                ...
                                shiny::fluidRow(
                                  column(width=4,
                                         conditionalPanel(condition = "input.Start_run",
                                                         ...
                                                         # uiOutput("ModOutp_a9"),
                                                          ...
                                                          tags$b("Results of CMSY analysis"),
                                                          uiOutput("ModOutp_b1"),
                                                          uiOutput("ModOutp_b2"),
                                                          uiOutput("ModOutp_b3"),
                                                          uiOutput("ModOutp_b4"),
                                                        ...
                                                        tags$b("Results for Management"),
                                                          ...
                                                          shinydashboard::box(collapsible = F,
                                                                              ...
                                                                              linebreaks(1),
                                                                              shinyWidgets::actionBttn(inputId="ppplot",label ="Prior/Posterior",
                                                                                                       style = "unite",size = "sm",icon = shiny::icon("chart-area"),
                                                                                                       no_outline=F,block=F,color="primary"),
                                                                              tags$hr(style = "border-top: 3px solid #000000;"),
                                                                              tags$b("Save a graph"),
                                                                              ...
                                                                              conditionalPanel(condition = "input.DL_picA",
                                                                                               ...
                                                                                               shinyWidgets::pickerInput(inputId = "Run_select_pic",
                                                                                                                         label = "",choices =c("A","B","C","D","E","F","G","H","I"),#,"K","I","J"
                                                                                                                         ...
                                                                                                                         choicesOpt = list(
                                                                                                                           subtext = c(
                                                                                                                             ...
                                                                                                                             "Kobe plot",
                                                                                                                             "Management graphs",
                                                                                                                             "CMSY prior, posterior distributions",
                                  ...
                                  column(width = 8,align="center",
                                           ...
                                           column(width = 12,align="center",offset=2,
                                                  conditionalPanel(condition = "input.Start_run",
                                                                   shinydashboard::box(collapsible = F,
                                                                                       ...
                                                                                       shiny::h5("The graph shows as light grey dots the explored log r-k space.  Dark grey dots are viable r-k pairs found to be compatible with catch data and the priors, with the most probable r-k pair indicted by the blue cross (with CI). The dotted rectangle indicates the prior r-k range."),
                                                                                       width = 8)))),#),height = 250
                                         shiny::fluidRow(
                                           conditionalPanel(condition = "input.Start_run",
                                                            shinydashboard::box(collapsible = F,
                                                                                shinycssloaders::withSpinner(shiny::plotOutput("Catch_plot_final")),
                                                                                shiny::h5("The plot shows as blue curve the catch and as dashed horizontal line the estimate of MSY, both with indication of 95% CI."),
                                                                                width = 12))),
                                           shiny::fluidRow(
                                            conditionalPanel(condition = "input.Start_run",
                                                            shinydashboard::box(collapsible = F,
                                         ...
                                         shiny::fluidRow(
                                           conditionalPanel(condition = "input.Start_run",
                                                            shinydashboard::box(collapsible = F,
                                                                                shinycssloaders::withSpinner(shiny::plotOutput("Pred_FFmsy_plot")),
                                                                                shiny::h5("Exploitation (F/Fmsy) as estimated (blue), where the dashed horizontal line indicates Fmsy. The grey dots are the observed harvest rate scaled to results."),
                                                                                width = 12))),#,height = 320
                                         ...
                                         shiny::fluidRow(
                                           conditionalPanel(condition = "input.Start_run",
                                                            shinydashboard::box(collapsible = F,
                                                                                ...
                                                                                shiny::h5("Surplus production equilibrium parabola, with reduced productivity at small stock sizes. Overlaid are the corresponding modeled results for relative catch and stock size (blue)."),
                                                                                width = 12))),
                                         shiny::fluidRow(
                                           column(width = 12,align="center",offset=1,
                                                  conditionalPanel(condition = "input.Start_run",
  ...
  #  shinyBS::toggleModal(session, "modalExample", toggle = "toggle")
    ...
    url_slfbs= a("SeaLifeBase", href="https://sealifebase.se/search.php",style = "color:black")
      ...
      shinydashboard::valueBox(
        shiny::h4("Create stock object - step 1."),shiny::h5("Provide some basic information about your stock (Species info and Region info).",br(),
                                                             ...
                                                             "Make sure that the scientific name matches exactly the scientific name used in ",url_fshbs," or ",url_slfbs, ", because this name will be used to extract information for the priors. If your species isn't in the dropdown list, type its name and press Add [Species Name]....") ,
    ...
    output$adv_up2=shinydashboard::renderValueBox({
      shinydashboard::valueBox(
        shiny::h4("Create stock object - step 2."),shiny::h5("Load your catch data from a comma-delimited (.csv) file.",
                                                             "That file should contain at least one column for Year, catch and, if available, cpue/biomass.",
    ...
    output$adv_up4=shinydashboard::renderValueBox({
      shinydashboard::valueBox(
        shiny::h4("Create stock object - step 3."),shiny::h5("The fourth step is give a unique name to your stock." ,br(),
      ...
      if(input$Crtd_StckID_input=="") {
        ...
        if(input$button_2 > 0) {
              newLine_catch_data <- isolate(data.frame(
                ...
                ct=inp_data()[,input$ct_col],
                bt=rep(NA,length(inp_data()[,input$Yr_col])),
    ...
    Catch_obj <- Catch_obj %>% debounce(500)
          ...
          color = "aqua") } else {
            shinydashboard::valueBox(
              shiny::h4("FishBase/SeaLifeBase info"),tagList(HTML(paste0("According to FishBase/SeaLifeBase, ", tags$em(Final_stock()[["Catch_ID"]][1,"ScientificName"]), " has "," Resilience: ",
                                                             tags$b(Fishbase_text()[2]), " Prior r range:", tags$b(Fishbase_text()[1]),tags$br(), "<font color=\"#cc0000\"><b>"," For more info press: ",url_r(), "</b></font>",tags$br(),
                                                             "The general receiving e-mail of FishBase and SeaLifeBase for comments, feedbacks and data sending to be added into the databases is info@quatics.org."
                                                             ))),
              icon = shiny::icon("info-circle"),
              color = "aqua")
          }
    ...
    output$Check_priors_plot= shiny::renderPlot({
      ...
      if (!isTruthy(input$file_LBB)) {
        p2= ggplot2::ggplot() +
          ggplot2::geom_errorbar(aes(x=start_yr, ymin=start[1], ymax=start[2],color="A"),  size=1)+
          ggplot2::geom_errorbar(aes(x=input$man_Bk_int_year_A, ymin=ind[1], ymax=ind[2],color="A"), size=1)+
          ggplot2::geom_errorbar(aes(x=end_yr, ymin=end[1], ymax=end[2],color="A"), size=1)+
  ...
  # output$test_orf=renderTable({bkpriors_plot()})
  ...
  p_1= ggplot2::ggplot(data=f_bio_obj,ggplot2::aes(x=yr)) +
    ...
    req(Final_stock())
      ...
      ABC_object[["input"]][["Input_parameters"]]$StartYear_bt=NA
      ABC_object[["input"]][["Input_parameters"]]$EndYear_bt=NA
      ABC_object[["input"]][["Input_parameters"]]$Ecreep_cpue=NA
      ABC_object[["input"]][["Input_parameters"]]$Ecreep_Year=NA
      ABC_object[["input"]][["Input_parameters"]]$Ecreep_value=NA
      ABC_object[["input"]][["Input_parameters"]]$Smooth_cpue=NA
      ABC_object[["input"]][["Input_parameters"]]$Smooth_K_cpue=NA
      ...
      ABC_object[["input"]][["Input_parameters"]]$btype=NA
      ########Input_data
   #  ABC_object[["input"]][["Input_data"]]=fstck_Catch_Data
    ...
    middle_outplts=eventReactive(input$Save_priors==T,{
      if (input$nbk_=="Only start") {
        ...
        row.names(outpts_m)=c("Analysis","Prior for r","Start B/k prior")#,"q prior","Interm. B/k prior","End B/k prior"
      } else if (input$nbk_=="Start & intermediate") {
        outpts_m=data.frame(range=c(paste0(ABC_object_()[["input"]][["Input_parameters"]]$StartYear, "-", ABC_object_()[["input"]][["Input_parameters"]]$EndYear),
                                    paste0(ABC_object_()[["input"]][["Input_parameters"]]$r.low, "-",ABC_object_()[["input"]][["Input_parameters"]]$r.hi),
                                    paste0(ABC_object_()[["input"]][["Input_parameters"]]$stb.low, "-",ABC_object_()[["input"]][["Input_parameters"]]$stb.hi),
        ...
        row.names(outpts_m)=c("Analysis","Prior for r","Start B/k prior","Interm. B/k prior")#,"q prior"
      } else {
        outpts_m=data.frame(range=c(paste0(ABC_object_()[["input"]][["Input_parameters"]]$StartYear, "-", ABC_object_()[["input"]][["Input_parameters"]]$EndYear),
                                    paste0(ABC_object_()[["input"]][["Input_parameters"]]$r.low, "-",ABC_object_()[["input"]][["Input_parameters"]]$r.hi),
                                    paste0(ABC_object_()[["input"]][["Input_parameters"]]$stb.low, "-",ABC_object_()[["input"]][["Input_parameters"]]$stb.hi),
        ...
        year=c("","","",ABC_object_()[["input"]][["Input_parameters"]]$int.yr,""))#,""
        row.names(outpts_m)=c("Analysis","Prior for r","Start B/k prior","Interm. B/k prior","End B/k prior")#,"q prior"
    ...
    observeEvent(input$Save_priors, {
     req(object_NAME())
      ...
      Save_done <- showNotification(paste("Message:",  "The stock object with the input parameterization has been saved in ", paste0(dir.name(),"/CMSY/",object_NAME(), ".RData")), duration = 10)
    ...
    output$select_diff_obj <- renderUI({
      filenames <- list.files(paste0(dir.name(),"/CMSY"), pattern="*.RData", full.names=F)
    ...
    fstc=reactive({
      if (input$Id049=="A") {
        sdsd=ABC_object_() } else if (input$Id049=="B") {
          asa=paste0(dir.name(),"/CMSY/",input$Id081,".RData")
         load(asa)
          sdsd=ABC_object
      }
      return(sdsd)
  ...
  xxx=xxx[,c(2,1)]
    ...
    q_priors= eventReactive(input$Start_run,{
      req(fstc())
      req(r.k_priors())
      q.priors_=q.priors(fstc(),r.k_priors(),"CMSY")
      return(q.priors_)
    })
   ##################### RUN THE MODEL
    ...
    CMSY_run=eventReactive(input$Start_run,{
      ABC_fit(fstc(),r.k_priors(),q_priors(),"CMSY")
    })
    ...
    ABC_object_final=eventReactive(input$Start_run,{
      ABC_object_final_=extract_ABC_fit(CMSY_run(),q_priors(),fstc())
    })
    ...
    observeEvent(input$Start_run,{
      run_pictures$pic_A=ggrk.plot(ABC_object_final(),r.k_priors(),CMSY_run(),"CMSY")
    ...
    # output$ModOutp_a9=renderUI({HTML(paste0(tags$b("Multivariate prior range for r: "),format(r.k_priors()[["rkpriors"]]$init.r[1],digits = 3), "-", format(r.k_priors()[["rkpriors"]]$init.r[2],digits = 3)))})

     output$ModOutp_a10=renderUI({HTML(paste0(tags$b("Derived prior range for k: "),format(round(r.k_priors()[["rkpriors"]]$prior.k.low),digits = 3), "-", format(round(r.k_priors()[["rkpriors"]]$prior.k.hi),digits = 3)))})
    output$ModOutp_a11=renderUI({HTML(paste0(tags$b("Derived prior MSY: "),format(round(as.numeric(ABC_object_final()[["input"]][["Input_parameters"]]$MSY_prior)),digits = 3)))})
   ...
   ############ Catch plot
     observeEvent(input$Start_run,{
      run_pictures$pic_B=ggcatch.plot(ABC_object_final(),METHOD = "CMSY")
    ...
    observeEvent(input$Start_run,{
      run_pictures$pic_C=ggbk.plot(ABC_object_final(),"CMSY")
...
###### FFmsy PLOT
    observeEvent(input$Start_run,{
      run_pictures$pic_D=ggFFmsy.plot(ABC_object_final(),"CMSY")
...
###### PARABOLA PLOT
        observeEvent(input$Start_run,{
      run_pictures$pic_E=ggparabola.plot(ABC_object_final(),"CMSY")
    ...
    ###### KOBE PLOT
    observeEvent(input$Start_run,{
      run_pictures$pic_F=ggkobe.plot(ABC_object_final(),CMSY_run() ,"CMSY")
    ...
    output$kobe_plot= shiny::renderPlot({
      ...
      observeEvent(input$Start_run,{
      run_pictures$pic_G= gg_management.plot(ABC_object_final(),CMSY_run(),"CMSY")
    ...
    observeEvent(input$Start_run,{
      run_pictures$pic_H= ggprorposterior.plot(ABC_object_final(),CMSY_run(),r.k_priors(),"CMSY")
      pic_G_ready <- showNotification(paste("Message: ", "Prior Posterior plot graph ready"), duration = 5)
    })
    ...
    observeEvent(input$button_summary, {
      ...
      xxx= gg_summary.plot(ABC_object_final(),r.k_priors(),CMSY_run(),"CMSY")
      
      ggsave(filename=paste0(paste0(dir.name(),"/CMSY/outputs/",nm,"/","Summary_pic."),device_),plot=xxx, device =device_, width = 25, height =18, units = "cm",  dpi = 300)
    ...
    ABC_retro=eventReactive(input$Retrospective,{
      req(ABC_object_final())
      xx= retro_fit(ABC_object_final(),r.k_priors(),q_priors(),input$retro_range,"CMSY")
    })

    observeEvent(input$Retrospective,{
      req(ABC_retro())
      run_pictures$pic_I=ggplot.retro(ABC_retro())
    })
    ...
    observeEvent(input$Start_run, {
     ...
     ABC_object <- ABC_object_final()
       ...
       nm=object_NAME()} else if (input$Id049=="B") {
         nm=input$Id081}
       dir.create(paste0(dir.name(),"/CMSY/outputs"))
       dir.create(paste0(dir.name(),"/CMSY/outputs/",nm))
        device_="tiff"
       ggsave(filename=paste0(paste0(dir.name(),"/CMSY/outputs/",nm,"/","rk_pic."),device_),plot=run_pictures$pic_A, device =device_, width = 16, height =10, units = "cm",  dpi = 300)
       ggsave(filename=paste0(paste0(dir.name(),"/CMSY/outputs/",nm,"/","Catch_and_MSY."),device_),plot=run_pictures$pic_B, device =device_, width = 16, height =10, units = "cm",  dpi = 300)
       ggsave(filename=paste0(paste0(dir.name(),"/CMSY/outputs/",nm,"/","Stock_size."),device_),plot=run_pictures$pic_C, device =device_, width = 16, height =10, units = "cm",  dpi = 300)
       ggsave(filename=paste0(paste0(dir.name(),"/CMSY/outputs/",nm,"/","Exploitation_rate."),device_),plot=run_pictures$pic_D, device =device_, width = 16, height =10, units = "cm",  dpi = 300)
       ggsave(filename=paste0(paste0(dir.name(),"/CMSY/outputs/",nm,"/","Equilibrium_curve."),device_),plot=run_pictures$pic_E, device =device_, width = 16, height =10, units = "cm",  dpi = 300)
       ggsave(filename=paste0(paste0(dir.name(),"/CMSY/outputs/",nm,"/","Kobe_plot."),device_),plot=run_pictures$pic_F, device =device_, width = 16, height =10, units = "cm",  dpi = 300)
       ggsave(filename=paste0(paste0(dir.name(),"/CMSY/outputs/",nm,"/","Management_graphs."),device_),plot=run_pictures$pic_G, device =device_, width = 16, height =10, units = "cm",  dpi = 300)
       ggsave(filename=paste0(paste0(dir.name(),"/CMSY/outputs/",nm,"/","prior_posterior_distributions."),device_),plot=run_pictures$pic_H, device =device_, width = 16, height =10, units = "cm",  dpi = 300)
       save(ABC_object,  file =paste0(dir.name(),"/CMSY/outputs/",nm,"/","ABC_obj_",gsub(" ", "_",fstc()[["input"]][["Stock_info"]][["ScientificName"]]), "_",Sys.Date(), ".RData"))
       write.csv(cbind(fstc()[["input"]][["Stock_info"]],fstc()[["input"]][["Input_parameters"]]), paste0(dir.name(),"/CMSY/outputs/",nm,"/input_parameters.csv"), row.names = F)
       write.csv(ABC_object[["output"]][["output_timeseries"]], paste0(dir.name(),"/CMSY/outputs/",nm,"/output_timeseries.csv"), row.names = TRUE)
       write.csv(ABC_object[["output"]][["output_posteriors"]], paste0(dir.name(),"/CMSY/outputs/",nm,"/output_posteriors.csv"), row.names = TRUE)
