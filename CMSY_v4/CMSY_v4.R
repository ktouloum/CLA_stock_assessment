...
test_CATCH_ID= tibble::as_tibble(test_CATCH_ID[,c("Continent","Region","Subregion","Group","ScientificName" ,"Stock")])
  ...
  #  shinyBS::toggleModal(session, "modalExample", toggle = "toggle")
    ...
    created_stock_object=eventReactive(input$button_2,{
      if(input$Crtd_StckID_input=="") {
        ...
        if(input$button_2 > 0) {
          Creatd_Stck_obj=CLA_stock_maker_empty_shiny(inp_data(),input$Crtd_StckID_input,input$Yr_col,input$ct_col,"bt",input$cv_col,"No")#
  ...
  #Final_stock <- Final_stock %>% debounce(1000)
    ...
    observe({
      updateSliderInput(session, "man_Bk_int_year_A", value =as.integer(floor((Catch_obj_()[c("yr","ct_smthd")]$yr[which(Catch_obj_()[c("yr","ct_smthd")]$ct_smthd>0)[1]]+max(as.integer(Catch_obj_()$yr),na.rm = T))/2)),min = min(Catch_obj_()$yr), max =max(Catch_obj_()$yr))
    })
