...
responsive_font <- function(output_id, session,
                            min_size = 12,
                            max_size = 22,
                            divisor = 45) {
  w <- session$clientData[[paste0("output_", output_id, "_width")]]
  if (is.null(w) || is.na(w)) return(min_size)
  max(min_size, min(max_size, w / divisor))
}
...
Catch_obj=function(inp,start_yr=NA, end_yr=NA,smoother_bw=1,Catch_CV=0.15,Plot=T,ShowCV=T ) {
  ...
  if (ShowCV==F) {  
    p1= ggplot2::ggplot(data=ct_obj,ggplot2::aes(x=yr)) +
      ...
      ggplot2::labs(y="Catch", x="Year",title="Catch data",color="",fill="") 
  } else {
    p1=  ggplot2::ggplot(data=ct_obj,ggplot2::aes(x=yr)) +
      ggplot2::scale_y_continuous(limits = c(0,1.1*max(c(ct_obj$ct,upper_cv),na.rm = T)))+
      ggplot2::scale_x_continuous(limits = c(min(ct_obj$yr,na.rm=T)-1,max(ct_obj$yr,na.rm=T)+1),breaks = breaks_pretty(n=brks))+
...
ggparabola.plot= function(CLA.obj,METHOD) {
  ...
  pic_parabola=ggplot2::ggplot()+
    ...
    ggplot2::scale_x_continuous(limits=c(0,1),breaks = breaks_pretty(n=nyr/5))+
...
ggkobe.plot= function(CLA.obj,fit.obj,METHOD,Management=F) {
  ...
  picKobe=ggplot2::ggplot()+
    ggplot2::scale_x_continuous(limits=c(0,max.x),breaks=seq(0,max.x,0.5),expand = c(0,0)) + 
    ggplot2::theme_classic()+
    ggplot2::scale_y_continuous(limits=c(0,max.y),breaks=seq(0,max.y,0.5),expand = c(0,0)) +
...
CLA.summary =function(CLA.obj) {
  ...
  cat("Catch data Smoothing: ",ifelse(CLA.obj[["input"]][["Input_parameters"]]$Smooth_K_catch==1,"NO",paste0("Yes, smoothing factor: ",
                                                                                                             CLA.obj[["input"]][["Input_parameters"]]$Smooth_K_catch)), "\n")
  cat("Catch data CV: ",CLA.obj[["input"]][["Input_parameters"]]$CV_catch , "\n")
  
  cat("CPUE data used from years: ",CLA.obj[["input"]][["Input_parameters"]]$StartYear_bt,
      "-", CLA.obj[["input"]][["Input_parameters"]]$EndYear_bt,"\n")
  
  cat("CPUE data Smoothing: ",ifelse(CLA.obj[["input"]][["Input_parameters"]]$Smooth_K_cpue==1,"NO",paste0("Yes, smoothing factor: ",
                                                                                                           CLA.obj[["input"]][["Input_parameters"]]$Smooth_K_cpue)), "\n")
  
  
  cat("CPUE data technoligical creep: ",ifelse(CLA.obj[["input"]][["Input_parameters"]]$Ecreep_cpue==F,"NO",
                                               paste0("Yes, starting year: ",CLA.obj[["input"]][["Input_parameters"]]$Ecreep_Year, 
                                                      ", technoligical creep percentage: ", CLA.obj[["input"]][["Input_parameters"]]$Ecreep_value, "%")), "\n")
  
  cat("CPUE data CV: ",CLA.obj[["input"]][["Input_parameters"]]$CV_cpue , "\n")
  ...
  cat("B/B0 in last year  =",format(tail(CLA.obj[["output"]][["output_timeseries"]]$bk,1),digits = 3),
      ", 2.5th perc =",format(tail(CLA.obj[["output"]][["output_timeseries"]]$bk_low,1),digits = 3),", 97.5 perc =",
      format(tail(CLA.obj[["output"]][["output_timeseries"]]$bk_high,1),digits = 3),"\n")
  ...
  cat("Catch in last year  =",format(tail(CLA.obj[["output"]][["output_timeseries"]]$Catch,1),digits = 3),
      ", 2.5th perc =",format(tail(CLA.obj[["output"]][["output_timeseries"]]$Catch_low,1),digits = 3),", 97.5 perc =",
      format(tail(CLA.obj[["output"]][["output_timeseries"]]$Catch_high,1),digits = 3),"\n")
